...
| 变更类型 | 处理方式 |
|---------|---------|
| 映射内新增文件 | 自动合并（写入本地 + `git add`） |
| 映射内重命名 | 自动合并（写新路径、删旧路径） |
| 小变更 (< 30 行) | 自动合并 |
| 大变更 (>= 30 行) | 人工审阅（保留本地） |
| 有本地修改 | 人工审阅 |
| 文件被删除 | 人工审阅（**保留本地**，不自动删） |
| SUMMARY.md（任意深度） | 始终人工审阅 |
| 无映射的上游路径 | **不同步**；报告列出并给出建议 mapping |

## 无映射新路径

上游新增顶层目录时，若 `.upstream-mapping.json` 无对应规则：

1. 终端列出每个未映射路径及类型（`added` 等）
2. 按现有映射推断建议条目（如本地前缀 `docs/zh/`）
3. 写入 `sync-conflict-report.md` 的 **Unmapped Upstream Paths** 节
4. 将建议条目加入 mapping 后重新运行同步即可拉取
